...
std::span<const float> AudioAnalyzer_new::compute_amplitudes(const FrequencyAnalyzer &fa)
{
...
std::span<const float> AudioAnalyzer_new::compute_phase(const FrequencyAnalyzer &fa)
{
	if (!fft_phase.empty())
		return fft_phase;
...
AudioAnalyzer_new::FrequencyAmplitudePair AudioAnalyzer_new::compute_peak_frequency(int from_hz, int to_hz)
{
	if (peak_freq_amp)
...
AudioAnalyzer_new::compute_multiband_shake(int from_hz, int to_hz)
{
	if (multiband_shake)
